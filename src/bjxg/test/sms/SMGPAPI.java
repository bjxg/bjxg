...
import com.simpleteam.connection.protocol.smgp.SMGPConnection;
...
import com.simpleteam.packet.smgp.SMGPDeliver;
...
import com.simpleteam.packet.smgp.SMGPSubmit;
...
public class SMGPAPI {
    // Class Name
    private final static String CLASS_NAME = "SMGPAPI";
     ...
     *         </p>
     */
    public SMGPAPI(String enterpriseCode, String serviceCode, String account, String password) {
        // Create sequence.
        sequence = new SimpleSequence();
        // Create connection.
        connection = new SMGPConnection();
    ...
    private boolean login() {
        ...
        if (!login.isValid()) {
            ...
            if (LogRequests.isRequested(EventID.SESSION | EventID.EXCEPTION)) {
                SimpleLog.log(CLASS_NAME, "login").message("invalid SMGP login !").end();
            }
            return false;
        }
    ...
    public void close() {
        ...
        if (!connection.isClosed()) {
            ...
            if (!connection.isTerminated()) {
                ...
                if (!authenticated) {
                    /*
                     * //Log event. if(LogRequests.isRequested(EventID.SMGP_PACKET | EventID.INFORMATION))
    ...
    public static void main(String[] args) {
        ...
        // Enterprise code.
        String enterpriseCode = "3001999999";
        ...
        if (!logDirectory.exists() && !logDirectory.mkdir()) {
            System.out.println("SMGPAPI.main : fail to create log directory !");
            return;
        }
        // Log mode.
        ...
        if (args.length > 1) {
            ...
            // Print.
            System.out.println("SMGPAPI.main : log mode(0x" + Hex.toString(logMode) + ") !");
        }
        // Initialize log.
        if (!SimpleLog.initialize(logDirectory, logMode)) {
            System.out.println("SMGPAPI.main : fail to initialize log !");
            return;
        }

        try {
            // Create API.
            SMGPAPI smgpApi = new SMGPAPI(enterpriseCode, serviceCode, account, password);
            // Connect.
            if (smgpApi.connect("127.0.0.1", 9890)) {
                // Print.
                System.out.println("SMGPAPI.main : connection was built !");
                ...
                while (reportCount < totalCount) {
                    ...
                    if (sendCount < totalCount) {
                        // Create submit.
                        SMGPSubmit submit = new SMGPSubmit(smgpApi.nextSequence());
                        ...
                        // Write packet.
                        smgpApi.writePacket(submit);
                        // Add send count and clear flag.
                        sendCount++;
                    }

                    // Check available.
                    if (smgpApi.available()) {
                        // Read packet.
                        SMGPPacket packet = smgpApi.readPacket();
                        // Check instance.
                        if (packet instanceof SMGPDeliver) {
                            // Get deliver.
                            SMGPDeliver deliver = (SMGPDeliver) packet;
                            ...
                            // Write response.
                            smgpApi.writePacket(response);// Do response as soon as possible.
                            ...
                            if ((deliver.is_report & 0x0f) == 1) {
                                // Print.
                                System.out.println("SMGPAPI.main : report/status(" + deliver.stat + ")");
                            }
                            else {
                                // Print.
                                System.out.println("SMGPAPI.main : normal/source(" + deliver.src_term_id + ")");
                            }
                            //
                            // /////////////////////////////////////////////////////////////////////////
                        }
                        ...
                        // Check command.
                            ...
                            // Print.
                            System.out.println("SMGPAPI.main : submit/result(" + response.msg_id + ")");
                ...
                // Print.
                System.out.println("SMGPAPI.main : fail to connect !");
            }
            // Close.
            smgpApi.close();
            // Print.
            System.out.println("SMGPAPI.main : connection was closed !");
        }
        catch (Exception e) {
            // Print.
            System.out.println("SMGPAPI.main : " + e.getMessage());
            System.out.println("SMGPAPI.main : unexpected exit !");
        }
